...
int ansi_read(char *ansi_file_name)
{
    FILE *ansfile = NULL;
    char *bufptr = NULL;
    char *scanptr = NULL;
    ...
    printf("ansi_file_name = [%s]\n", ansi_file_name);

    ansfile = fopen(ansi_file_name, "rb");

    if (!ansfile) {
        printf("cannot open: %s: %s\n", ansi_file_name, strerror(errno));
        exit(1);
    }
    ...
    printf("[%lu] bytes read to buffer\n", bytes_read);
    ...
    while (offset < bytes_read) {
        ...
        switch(ansi_mode) {
        case SEQ_ERR:
            printf("--\n0x%04lx: error in state, ansi_mode = [%s], last_ansi_mode = [%s], character = 0x%02X '%c'\n", current_escape_address,
                   (const char *) ansi_state(ansi_mode), (const char *) ansi_state(last_ansi_mode), last_c, (last_c < 32 ? '.' : last_c));
            printf("ANSIBUF[%d@0x%08lx]: ", ansioffset, current_escape_address);
        ...
        case SEQ_ANSI_EXECUTED:
            ...
            break;
        default:
            printf("--\n0x%04lx: error in state [UNHANDLED], ansi_mode = [%s], last_ansi_mode = [%s], character = 0x%02X\n", offset,
